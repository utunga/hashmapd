...
def test_SMH( finetune_lr = 0.3, pretraining_epochs = 100, \
    ...
    output_trace_info(smh, datasets,'fake_b4_finetuning')
    ...
    output_trace_info(smh, datasets,'fake_after_finetuning')
...
WEIGHTS_FILE='data/last_smh_model_params.pkl.gz'
def save_model(smh):
    save_file=open(WEIGHTS_FILE,'wb')
    cPickle.dump(smh.params, save_file, cPickle.HIGHEST_PROTOCOL);
    save_file.close();

#something wrong with this so commenting out 
#def load_model(smh):
#    save_file=open(WEIGHTS_FILE)
#    smh.params = cPickle.load(save_file)
#    save_file.close()
#    return smh
#
#def load_smh_from_weights_file(n_ins=784):
#    numpy_rng = numpy.random.RandomState(212)
#    smh = SMH(numpy_rng = numpy_rng, n_ins = n_ins)
#    save_file=open(WEIGHTS_FILE)
...
if __name__ == '__main__':
    ...
    # test on 'fake img data' (14 input cases )
    #smh = test_SMH(dataset=FAKE_IMG_DATA_FILE, batch_size=1, \
    #              pretraining_epochs = 500, training_epochs = 500, n_ins=NUM_PIXELS)
    #save_model(smh)
   
    smh = test_SMH(dataset=UNSUPERVISED_MNIST, batch_size=8, \
                  pretraining_epochs = 100, training_epochs = 100, mid_layer_size=200, inner_code_length=30, n_ins=NUM_PIXELS)
    save_model(smh)
   
    
    # dunno why but this doesnt work
    #smh = load_smh_from_weights_file(n_ins=NUM_PIXELS)
    #datasets = load_data(dataset=FAKE_IMG_DATA_FILE)
    #output_trace_info(smh, datasets, 'test_weights_restore')

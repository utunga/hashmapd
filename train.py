...
def train_smh(finetune_lr = 0.3, 
        ...
        k = 1, 
        training_epochs = 5, 
        dataset=os.path.join('data', 'truncated_mnist.pkl.gz'), 
        batch_size = 10, 
        mid_layer_sizes = (200,), 
    ...
    for i in xrange(smh.n_rbm_layers):
        ...
        for epoch in xrange(pretraining_epochs):
            ...
            for batch_index in xrange(n_train_batches):
                batch_cost = pretraining_fns[i](index = batch_index, 
                    lr = pretrain_lr )
    ...
    patience              = 4*n_train_batches # look at this many examples
    ...
    while epoch < training_epochs and not done_looping:
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            iters = epoch * n_train_batches + minibatch_index
...
def output_trace_info(smh, datasets, prefix, skip_trace_images):
    ...
    for layer in xrange(smh.n_sigmoid_layers):
        ...
        sigmoid_layer.export_weights_image(
            os.path.join('trace', '%s_weights_%i.png'%(prefix,layer)))
    ...
    image.save(os.path.join('trace', '%s_input.png' % prefix))
    ...
    image.save(os.path.join('trace', '%s_reconstruction.png' % prefix))


DEFAULT_WEIGHTS_FILE = os.path.join('data', 'last_smh_model_params.pkl.gz')
...
def load_model(n_ins = 784,  
    ...
    smh = SMH(numpy_rng=numpy_rng,  
        ...
        inner_code_length=inner_code_length, 
        n_ins=n_ins)
    smh.unroll_layers() #need to unroll before loading model
